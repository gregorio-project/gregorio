...
\noindent\textbf{NABC neumes in running text}
...
without a clef, optionally accompanied by nabc notation above (voice~1)
...
\verb|\nabcneumesnippet| typesets nabc glyphs on their own, with no square
notation at all.  The font and its size are those of the nabc voice given as
optional argument, so \verb|\grenabcfont|, which selects them for the current
\TeX\ group only, can be used to pick them on the spot:
