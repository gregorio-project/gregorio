...
Macro to create one of Gregorio\TeX’s distances.  Used to initialize distances in a space configuration file.  See \textit{gsp-default.tex} for an example of such a file.
...
\begin{argtable}
  \#1 & \texttt{bridge} & Adjacent horizontal episema are joined together (default).\\
  & \texttt{break} & Adjacent horizontal episema are not joined.
\end{argtable}
...
\begin{argtable}
  \#1 & \texttt{1/2/3/4/5} & Choose the number of the line desired\\
...
\begin{argtable}
  \#1 & \texttt{1/2} & Choose the number of the ornamentation desired\\
  \#2 & integer & the point size at which to print the line\\
\end{argtable}
