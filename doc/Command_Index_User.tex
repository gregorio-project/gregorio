...
Minimum gap between the NABC neumes of consecutive syllables.  NABC neumes are typeset in zero-width boxes and may extend past the notes of their syllable; when they do, the syllable-spacing algorithm widens the space after the syllable just enough to keep this minimum separation from the next syllable's neumes (in the same NABC voice).  The default value (0.06\,cm) is 25\% of \texttt{intersyllablespacenotes} and provides a comfortable visual margin.  Set it to \texttt{0pt} to allow the neumes of consecutive syllables to touch.
\end{gdimension}
