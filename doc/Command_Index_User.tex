...
\macroname{\textbackslash includescore[\#1]\{\#2\}}{gregoriotex.tex}
%\verb=\includescore#1=%tex.tex
Macro for including scores.  Works on both gabc and tex files.

\begin{argtable}
  \#1 & character& Optional.  Should be ``\texttt{f}''.  Bypasses compilation checks.\\
  \#2 & string & Relative or absolute path to the score.\\
\end{argtable}

Example:\par\medskip
\begin{latexcode}
  \includescore[f]{TecumPrincipium.tex}
...
Changes settings to align syllables in the Latin style (center of vowel aligns with center of first glyph in neume).  Will only affect scores which do not have “centering-scheme: english;” in their gabc header.  
