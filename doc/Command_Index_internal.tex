...
\begin{argtable}
  ...
  & \texttt{3} & case of the vertical episemus, which is not placed at the same place if the corresponding note is on a line or not\\
  & \texttt{4} & case of the punctum mora, for the same reason\\
  & \texttt{5} & case of the horizontal episemus under a note, that must be placed a bit lower if the note is on a line\\
  ...
  & \texttt{9} & case of the horizontal episemus, that must be placed a bit lower if the note is on a line\\
...
\begin{argtable}
  ...
  \#2 & \texttt{0} & go back to the beginning of the previous glyph; this starts the episemus at the beginning of the previous glyph\\
  & \texttt{1} & stay at the end of the glyph; doesn’t make much sense to use this\\
  & \texttt{2} & go back the width of \#1; this starts the episemus at the glyph from the end that starts at \#1’s width from the end\\
  & \texttt{3} & go back to the beginning of the previous glyph and then forward the width of \#1; this starts the episemus at the glyph from the start that starts just after \#1’s width from the start\\
  & 4 & go back to the beginning of the previous glyph and then forward the width of \#1, then back the width of \#2; this ends the episemus at the end of \#1\\
  ...
  \#4 & \texttt{0} & an horizontal episemus\\
  & \texttt{1} & an horizontal episemus under a note\\
  & \texttt{2} & a line at the top\\
  & \texttt{3} & a line at the bottom\\
  \#5 & \texttt{f} & a normal episemus\\
  & \texttt{l} & a small episemus aligned left\\
  & \texttt{c} & a small episemus aligned center\\
  & \texttt{r} & for a small episemus aligned right\\
\end{argtable}
...
\begin{argtable}
  ...
  \#3 & \texttt{0} & go back to the beginning of the previous glyph; this starts the  episemus at the beginning of the previous glyph\\
  & \texttt{1} & stay at the end of the glyph; doesn’t make much sense to use this\\
  & \texttt{2} & go back the width of \#1; this starts the episemus at the glyph from the end that starts at \#1’s width from the end\\
  & \texttt{3} & go back to the beginning of the previous glyph and then forward the width of \#1; this starts the episemus at the glyph from the start that starts just after \#1’s width from the start\\
  \#4 & \texttt{0} & an horizontal episemus\\
