...
Renders NABC neumes without preserving any note glyph or inter-element spacing.  Uses \verb=\gre@nabconlyglyph= instead of \verb=\GreGlyph= and suppresses all end-of-element and end-of-glyph callbacks, as well as bar output.  The horizontal space the NABC needs is provided by the syllable-spacing Lua filter, which enforces \texttt{nabcintersyllablemingap} between the NABC neumes of consecutive syllables.  This produces tighter spacing than the hphantom path.
...
Phase 1 of NABC two-phase processing: pre-evaluates NABC content, emits note-level kern for left overflow (in \texttt{neume} alignment mode) unless \verb=\ifgre@nabc@suppress@leftoverflow= is true, and saves the rendered content in a box register.
