...
Macro to impose styling on the first syllable of a score (except for the score initial).

\begin{argtable}
  \#1 & string & The syllable to be styled.
\end{argtable}

\macroname{\textbackslash GreFirstSyllableInitial}{\#1}{gregoriotex-syllable.tex}
Macro to impose styling on the first letter of the first syllable of a score which is not the score initial (\ie, the second letter of the score for scores with initials, the first letter of the score for scores without them).

\begin{argtable}
  \#1 & character & The character to be styled.
\end{argtable}
...
\macroname{\textbackslash GreGlyph}{\#1\#2\#3\#4\#5\#6}{gregoriotex-syllable.tex}
...
\begin{argtable}
  ...
  \#3 & character & height of the next note\\
...
\begin{argtable}
  ...
  \#6 & character & Replacement for \#1 if a bridge causes a height substitution.\\
\end{argtable}
...
\macroname{\textbackslash GreLineaPunctumCavum}{\#1\#2\#3\#4\#5}{gregoriotex-signs.tex}
...
\begin{argtable}
  \#1 & character & height of the custo
\end{argtable}

\macroname{\textbackslash GreMode}{\#1}{gregoriotex-main.tex}
If the gabc file contains a mode in the header, then this function
places said mode as the first (top) annotation.  If the user has manually added a first annotation in the \TeX\ file, then this function does nothing.
...
\begin{argtable}
  \#1 & character & Height of the natural.\\
  \#2 & 0         & No a flat for a key change.\\
      & 1         & Indicates the a flat for a key change.\\
\end{argtable}
...
\macroname{\textbackslash GreNewLineWithSpace}{\#1\#2\#3\#4}{gregoriotex-main.tex}
Macro called to go to the next line but when there are additional vertical spaces to add

\begin{argtable}
  \#1 & 0 & No note above staff.\\
      & 1 & Note above 4th line (position \texttt{k}).\\
      & 2 & Note on 5th line (position \texttt{l}).\\
      & 3 & Note above 5th line (position \texttt{m}).\\
  \#2 & 0 & No note below staff.\\
      & 1 & Note below 1st line (position \texttt{c}).\\
      & 2 & note on 0th line (position \texttt{b}).\\
      & 3 & Note below 0th line (position \texttt{a}).\\
      & 4 & note below 0th line (position \texttt{a}) with vertical episemus.\\
  \#3 & 0 & No translation.\\
...
\macroname{\textbackslash GreNewParLineWithSpace}{\#1\#2\#3\#4}{gregoriotex-main.tex}
...
\macroname{\textbackslash GreSetGregorioFont}{\#1}{gregoriotex-main.tex}
Macro to set the font used for the glyphs.

\begin{argtable}
  \#1 & gregorio & Use gregorio font.\\
  & parmesan & Use parmesan font.\\
  & greciliae & Use greciliae font.\\
\end{argtable}
...
\begin{argtable}
  ...
  \#2 & 1--4       & Line of key.\\
  \#3 & character & Height of flat in key (\texttt{a} for no flat).\\
\end{argtable}
...
\begin{argtable}
  ...
  & 1 & Space after clef.\\
  \#4 & character & Height of flat in key (\texttt{a} for no flat).\\
\end{argtable}
...
\begin{argtable}
  \#1 & character & Height of the sharp.\\
  \#2 & 0         & No a flat for a key change.\\
      & 1         & Indicates the a flat for a key change.\\
\end{argtable}
