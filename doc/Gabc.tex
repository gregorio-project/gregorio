...
Roman numeral using the \texttt{modeline} style.  However, there are other
modes.  If the first character of \texttt{mode} is a number from one
(\texttt{1}) through eight (\texttt{8}), that number will be converted
according to the \verb=\gresetmodenumbersystem= setting.  Everything else
will be typeset directly.  If the \texttt{mode} header is omitted, none of
the other mode headers will be typeset.
