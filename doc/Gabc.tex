...
Gregorio places horizontal episemus under c and above k closer to the notes when no ledger line is present.
The heuristics used by Gregorio are not perfect so it may be necessary to make
the presence or absence of ledger line explicit for horizontal episemus placement.
This is done in the exact same way as for stem length: place \texttt{[ll:0]} or
\texttt{[ll:1]} on the note carrying the episemus, to force gregorio to consider the
absence or presence of a ledger line in episemus placement.
