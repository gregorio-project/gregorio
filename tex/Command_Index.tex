...
\verb=normalstafflinesformat=%tex.sty, PlainTeX version in tex.tex
	Defines how the staff lines will appear.  Note that the staff lines are drawn with \verb=\hrule= and so very few things will actually affect their appearance (color being the most prominent one that does).
...
\verb=\GreSetStaffLinesFormat{<string>}=%tex.tex
	Deprecated.  Used to set the format for the staff lines.  See above notes on the environments for updated documentation.

\verb=\includescore{<filename>}=%tex.tex
	Macro for including scores.  Works on both gabc and tex files.

\verb=\includetexscore{<filename>}=%tex.tex
	Macro for including scores which have already been run through gregorio (i.e. are in gtex format).
	Deprecated version: \verb=\greincludetexscore=

\verb=\includegabcscore{<filename>}=%tex.tex
	Macro for including scores which are in gabc format.
...
\verb=\gre@includetexscore{<filename>}=%tex.tex
	Macro to include scores in gtex format.

\verb=\gre@includegabcscore{<filename>}=%tex.tex
...
\verb=\gre@falsepenalty{<int>}=%tex-signs.tex
	Does nothing.

\verb=\gre@truepenalty{<int>}=%tex-signs.tex
	Alias for \verb=\penalty{<int>}=.  Used in combination with \verb=\gre@falsepenalty= to avoid placing \verb=\penalty= where it isn't allowed.

\verb=\gre@hskip=%tex-signs.tex
	Alias for \verb=\hskip= outside of discretionaries and \verb=\kern= inside of them.

\verb=\gre@penalty{<int>}=%tex-signs.tex
...
\verb=\gre@setlinesclef{<char1>}{<char2>}{<int>}{<char3>}=%tex-signs.tex
	Macro to define the clef that will appear at the beginning of the lines.
	<char1>: type of clef (c or f)
	<char2>: height of clef (using gabc height letters)
	<int>: add a space after the clef (1), or not (0)
	<char3>: height of flat after clef (using gabc height letters, a = no flat)

\verb=\gre@calculateclefnum{<char1>}{<int>}{<char2>}=%tex-signs.tex
	Macro to calculate the clef number and store it in \verb=\gre@clefnum=.
	<char1>: type of clef (c or f)
	<int>: line on which the clef appears (1--4)
	<char3>: height of flat after clef (using gabc height letters, a = no flat)
