...
%% 6: case of the horizontal episemus under a note, that must be placed a bit lower if the note is on a line
%% 7: case of the signs above (accentus, etc.)
...
%%%%% 9 : flexus deminutus
\def\glyph#1#2#3#4{%
% first there is something we must do: setting \lastoflinecount to 0 if its value is 2. The reason of this is that in ab(abzcd), we cannot let \lastoflinecount set to 2 after the end of line... the only way I found to achieve it is to reset \lastoflinecount to 0 after each glyph or bar.
...
% we need to save lastoflinecount, because otherwise when a \newline appears inside a syllable, it is counted two times (once here and once later when #9 will be really output), so \lastoflinecount has a wrong value. Thus
\edef\savedlastoflinecount{\number\lastoflinecount\relax }%
\syllablenotes{#9}% we put the notes in a box, so that we have the width of it
\firstglyph=1 %
\global\lastoflinecount=\savedlastoflinecount\relax %
...
\ifnum\lastoflinecount=2\relax %
\ifnum\disableeolshifts=0\relax%
...
\advance\clefwidth by -\minimalspaceatlinebeginning %
\ifdim\temp <-\clefwidth %
\temp=-\clefwidth %
\fi %
\advance\clefwidth by \minimalspaceatlinebeginning %
\hbox{}%
\advance\temp by -\spaceafterlineclef %
\ifdim\the\temp < -\the\begindifference %
\temp=-\the\begindifference %
\fi %
\kern\temp %
...
\setbox\Syllabletext=\hbox{\gretextformat{#1#2#3\grehyph\relax}}%
\else %
\setbox\Syllabletext=\hbox{\gretextformat{#1#2#3-}}%
\fi %
\setenddifference{\wd\Syllablenotes}{\wd\Syllabletext}{\textaligncenter}{\notesaligncenter}{0}%
...
\fi %
% trick: we want to kern -enddifference if necessary before any change of line, which we can achieve only with this trick if the end of line is the last thing of the notes (it's the case of \lastoflinecount is 1).
% For the kern in this case, the base is to kern -\enddifference if it's negative, but we can kern a bit more for the text to get a bit more to the right, but not after the end of the scores minus \minimalspaceatlinebeginning
\ifnum\lastoflinecount=1\relax %
\ifdim\the\enddifference <0pt%
\temp=-\enddifference %
% we don't do it if it's the last syllable of a score or if the user disabled it
\ifnum\disableeolshifts=0\relax %
\ifnum\blockcusto=1\else %
\setbox\Tempwidth=\hbox{\custochar{g}}%
\tempwidth=\wd\Tempwidth %
\advance\temp by -\tempwidth %
\advance\temp by -\spacebeforecusto %
\fi %
\advance\temp by \minimalspaceatlinebeginning %
\fi %
\ifdim\the\temp <-\the\enddifference %
\temp=-\the\enddifference %
\fi %
\xdef\kernbeforeeol{\the\temp\relax}%
\fi %
\fi%
...
\penalty 10001\relax %
\fi%
\xdef\kernbeforeeol{0pt\relax}%
% see comments in the glyph macro to see why it's commented
%\ifnum\lastoflinecount=2\relax %
...
\setbox\Tempwidth=\hbox{#5#6}%
% /!\ warning: can be buggy...
\ifdim\wd\Tempwidth=0pt%
\endbeforebar{0}%
\else %
\endbeforebar{1}%
\fi %
\fi %
\global\firstsyllableofword=1\relax %
\else %
...
\shittytexworkaround{#4}%
\fi%
\global\notesaligncenter=0pt% very important, see flat and natural
\relax%
}
...
\ifcase#1 %
\global\firstsyllableofword=0\relax %
\or%
\endofword{0}%
\global\firstsyllableofword=1\relax %
\fi%
...
\def\abovelinestextstyle#1{\small\itshape #1}
\newdimen\abovelinestextraise % raises the text above the note line
\newdimen\abovelinestextwidth % used for kerning to ``reset'' spacing
\newbox\abovelinestextbox % box for the text above

% set space above the text lines - almost the same as for the translation
\def\addspaceabove{%
\global\abovelinestextraise=60000 sp%
\global\multiply\abovelinestextraise by \the\grefactor %
%\global\advance\spaceabovelines by \abovelinestextraise %
\generatelines %
\calculateconstantglyphraise %
\global\advance\abovelinestextraise by 4\stafflineheight %
\global\advance\abovelinestextraise by 4\interstafflinespace %
\relax %
}
% we don't need space above any more
\def\removespaceabove{%
\global\abovelinestextraise=0 sp%
\generatelines %
\calculateconstantglyphraise %
\relax %
}
% set the text above the line
\def\abovelinestext#1{%
\setbox\abovelinestextbox\hbox{\abovelinestextstyle{#1}}
\leavevmode\raise\abovelinestextraise\copy\abovelinestextbox%
\global\abovelinestextwidth=\wd\abovelinestextbox%
\kern-\abovelinestextwidth%
}
