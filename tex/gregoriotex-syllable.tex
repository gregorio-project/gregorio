...
%% 11: case of the choral sign in the case of the first note of a podatus or the 2nd note of a porrectus, etc.
\def\grecalculateglyphraisevalue#1#2{%
  ...
  \ifcase\greisonaline\or% isonaline = 1
    ...
    \fi%
  \fi%
  \advance\gretempdimcount by -7 %
  \greglyphraisevalue = 15750 sp %
  \multiply\greglyphraisevalue by \the\grefactor %
  \multiply\greglyphraisevalue by \the\gretempdimcount %
  ...
  \or\or\or%3: if it is a vertical episemus on a line, we shift it a bit higher, so that it's more beautiful
    ...
    \advance\greglyphraisevalue by \greaddedraisevalue %
  ...
  \or% 9: if it is an horizontal episemus not on a line, we put it a bit lower
    ...
    \multiply\greaddedraisevalue by \the\grefactor %
    \advance\greglyphraisevalue by \greaddedraisevalue %
  \or% 10: if it is a choral sign, we shift it a bit lower, of a user-defined value
    \advance\greglyphraisevalue by -\grechoralsignshift %
  \or% 11: idem
    \advance\greglyphraisevalue by -\grechoralsignshift %
  \fi%
  \advance\greglyphraisevalue by \greconstantglyphraise %
