...
\gre@declarefileversion{gregoriotex-syllable.tex}{4.1.0-beta3}% GREGORIO_VERSION
...
%% 9 : flexus deminutus
%% 10 : virgula
%% 11 : divisio minima, minor, maior or dominican bar
%% 12 : divisio finalis
%% 13 : no note
...
% in the new bar algorithm, we have to cheat a little bit in the case of (:z),
% we don't want to create a new line just after the bar, instead we delay
% it to the end of the algorithm. To do so, we override gre@newlinecommon using
% these:
...
\def\GreBarSyllable#1#2#3#4#5#6#7#8#9{%
  ...
  \ifgre@newbarspacing%
    ...
    % first, recomputing \gre@dimen@previousenddifference into \gre@dimen@temp@three
    % taking into account the punctum mora shift
    \gre@dimen@temp@three=\gre@dimen@previousenddifference %
    % gre@skip@temp@four is 0 or the punctum mora shift
    ...
    \ifgre@lastendswithmora %
      \ifcase\gre@count@shiftaftermora\or\or %
        \ifdim\wd\gre@box@syllabletext=0pt\relax %
          ...
          \advance\gre@dimen@temp@three by \gre@skip@temp@four %
        \fi %
      \or %
    ...
    % the end of the text:
    \ifdim\gre@skip@temp@four=0pt\else %
      \gre@debugmsg{barspacing}{adjustment for punctum mora: \the\gre@skip@temp@four}%
      \gre@debugmsg{barspacing}{adjustend previous enddifference (for punctum mora): \the\gre@dimen@temp@three}%
      \ifdim -\gre@skip@temp@four < \gre@dimen@previousenddifference %
        \gre@debugmsg{barspacing}{kern \the\gre@skip@temp@four for punctum mora adjustment}%
    ...
    \gre@debugmsg{barspacing}{previous syllable final skip: \the\gre@skip@minTextDistance}%
    \gre@skip@temp@three = \glueexpr\wd\gre@box@syllabletext + \gre@skip@minTextDistance\relax%
    ...
    \ifdim\wd\gre@box@syllabletext > 0pt\relax%
      ...
      \ifnum#4=1\relax %
        \ifgre@in@euouae %
          \gre@skip@syllablefinalskip=\gre@space@skip@interwordspacetext@bars@euouae\relax%
        \else %
          \gre@skip@syllablefinalskip=\gre@space@skip@interwordspacetext@bars\relax%
        \fi %
      \else %
    ...
    \fi%
    \gre@debugmsg{barspacing}{space required by text: \the\gre@skip@temp@three}%
    \gre@debugmsg{barspacing}{allocated text space: \the\gre@skip@temp@one}%
    \gre@debugmsg{barspacing}{allocated notes space: \the\gre@skip@temp@two}%
    % now we check the bar width
    %as with the text, we start by borrowing the minNotesDistance left over from the previous syllable
    \gre@skip@temp@four = \glueexpr\wd\gre@box@syllablenotes + \gre@skip@minNotesDistance\relax%
    %now, if there is a visible bar for this syllable, we calculate this syllable’s equivalent of minNotesDistance
    % gre@count@temp@one holds 0 if next is not a bar, 1 if it is, 2 if it starts with an alteration
    \gre@count@temp@one=0%
    \ifnum#7>9\relax %
      \ifnum#7<20\relax %
        \gre@count@temp@one=1\relax %
    ...
    \ifdim\wd\gre@box@syllablenotes > 0pt\relax%
      %as with the text, this calculation is similar to what \gre@calculate@syllablefinalskip does, but only for the notes, so we’ll use syllablefinalskip to store it
      \ifcase\gre@count@temp@one\relax %
        \ifnum#4=1\relax %
          \ifgre@in@euouae %
            \gre@skip@syllablefinalskip=\gre@space@skip@interwordspacenotes@euouae\relax%
          \else %
      ...
      \advance\gre@skip@temp@four by \gre@skip@syllablefinalskip%
    \fi%
    \ifdim\gre@skip@temp@two < \gre@skip@temp@four\relax%
      \gre@skip@temp@one = \glueexpr\gre@skip@temp@one + (\gre@skip@temp@four - \gre@skip@temp@two)\relax%
      \gre@skip@temp@two = \gre@skip@temp@four\relax%
    \fi%
    \gre@debugmsg{barspacing}{required space for notes: \the\gre@skip@temp@four}%
    \gre@debugmsg{barspacing}{allocated text space: \the\gre@skip@temp@one}%
    \gre@debugmsg{barspacing}{allocated notes space: \the\gre@skip@temp@two}%
    ...
    \gre@dimen@begindifference=\dimexpr(-\gre@dimen@temp@three - \gre@skip@temp@two + \gre@skip@temp@one)\relax%
    \gre@debugmsg{barspacing}{new begindifference approximation: \the\gre@dimen@begindifference}%
    \gre@debugmsg{barspacing}{left offset limit: \gre@space@dimen@maxbaroffsettextleft}%
    \gre@debugmsg{barspacing}{right offset limit: \gre@space@dimen@maxbaroffsettextright}%
    \ifdim\gre@dimen@begindifference > \gre@space@dimen@maxbaroffsettextright\relax%
      %the two elements are too far apart and the bar is before the text
      %we want to create a shift for the one with the larger available space
      \gre@debugmsg{barspacing}{non-zero shifts}%
      \ifdim\gre@skip@temp@one > \gre@skip@temp@two\relax%
        \gre@debugmsg{barspacing}{shift the text to the left}%
        \gre@dimen@temp@one = \dimexpr(-\gre@dimen@begindifference + \gre@space@dimen@maxbaroffsettextright)\relax%
        \gre@dimen@temp@two = 0pt\relax%
      \else%
        \gre@debugmsg{barspacing}{shift the bar to the right}%
        \gre@dimen@temp@one = 0pt\relax%
        \gre@dimen@temp@two = \dimexpr\gre@dimen@begindifference - \gre@space@dimen@maxbaroffsettextright\relax%
      \fi%
    \else\ifdim\gre@dimen@begindifference < -\gre@space@dimen@maxbaroffsettextleft\relax%
      %the two elements are too far apart and the text is before the bar
      %we want to create a shift for the one with the larger available space
      \gre@debugmsg{barspacing}{non-zero shifts}%
      \ifdim\gre@skip@temp@one > \gre@skip@temp@two\relax%
        \gre@debugmsg{barspacing}{shift the text to the right}%
        \gre@dimen@temp@one = \dimexpr-\gre@dimen@begindifference - \gre@space@dimen@maxbaroffsettextleft\relax%
        \gre@dimen@temp@two = 0pt\relax%
      \else%
        \gre@debugmsg{barspacing}{shift the bar to the left}%
        \gre@dimen@temp@one = 0pt\relax%
        \gre@dimen@temp@two = \dimexpr\gre@dimen@begindifference + \gre@space@dimen@maxbaroffsettextleft\relax%
      \fi%
    \else%
      ...
      \gre@dimen@temp@two = 0pt\relax%
    \fi\fi%
    % All this worry about shifts is superfluous if either the bar line or text isn’t present, so now we check for that and reset them to 0 if one is missing
    \ifdim\wd\gre@box@syllablenotes = 0pt\relax%
      \gre@debugmsg{barspacing}{no notes, shifts set to 0}%
      \gre@dimen@temp@one = 0pt\relax%
      \gre@dimen@temp@two = 0pt\relax%
    \fi%
    \ifdim\wd\gre@box@syllabletext = 0pt\relax%
    ...
    % now, if the shifts are too big, the will cause the bar element they apply to to overlap with the corresponding element in an adjacent syllable
    % we consider the offset limits set by the user to be ruling, so to account for this we allocate more space to the bar syllable to prevent these collisions
    \ifdim\gre@dimen@temp@one > \glueexpr(\gre@skip@temp@one-\gre@skip@temp@three/2)\relax\relax%
      \gre@debugmsg{barspacing}{text collision on right}%
      \ifdim\glueexpr(\gre@dimen@temp@one + \gre@skip@temp@three/2 - \gre@skip@temp@one) > \gre@space@dimen@baroffsetadditionalspacearoundbars\relax %
        \gre@dimen@temp@five = -\dimexpr(\gre@dimen@temp@one + \gre@skip@temp@three/2 - \gre@skip@temp@one)\relax %
      \fi %
      \advance\gre@skip@temp@two by \glueexpr(\gre@dimen@temp@one + \gre@skip@temp@three/2 - \gre@skip@temp@one)\relax%
      \gre@skip@temp@one = \glueexpr\gre@dimen@temp@one + \gre@skip@temp@three/2\relax%
    \fi%
    \ifdim-\gre@dimen@temp@one > \glueexpr(\gre@skip@temp@one-\gre@skip@temp@three/2)\relax\relax%
      \gre@debugmsg{barspacing}{text collision on left}%
      \advance\gre@skip@temp@two by \glueexpr(-\gre@dimen@temp@one + \gre@skip@temp@three/2 - \gre@skip@temp@one)\relax%
      \gre@skip@temp@one = \glueexpr-\gre@dimen@temp@one + \gre@skip@temp@three/2\relax%
    \fi%
    \ifdim\gre@dimen@temp@two > \glueexpr(\gre@skip@temp@two-\gre@skip@temp@four/2)\relax\relax%
      \gre@debugmsg{barspacing}{bar collision on right}%
      \advance\gre@skip@temp@one by \glueexpr(\gre@dimen@temp@two + \gre@skip@temp@four/2 - \gre@skip@temp@two)\relax%
      \gre@skip@temp@two = \glueexpr\gre@dimen@temp@two + \gre@skip@temp@four/2\relax%
    \fi%
    \ifdim-\gre@dimen@temp@two > \glueexpr(\gre@skip@temp@two-\gre@skip@temp@four/2)\relax\relax%
      \gre@debugmsg{barspacing}{bar collision on left}%
      \advance\gre@skip@temp@one by \glueexpr(-\gre@dimen@temp@two + \gre@skip@temp@four/2 - \gre@skip@temp@two)\relax%
      \gre@skip@temp@two = \glueexpr-\gre@dimen@temp@two + \gre@skip@temp@four/2\relax%
    \fi%
    ...
    %we need to account for the widths of the elements and the offsets
    %currently begindifference holds the kern from the nominal middle of the notes to the nominal middle of the text
    %we thus need to increase it by the half the width of the bar line, decrease it by half the width of the text, decrease it by the kern from the nominal to the true center of the bar line, and increase it by the kern from the nominal to the true center of the text
    \gre@debugmsg{barspacing}{Correcting begindifference}%
    \gre@dimen@begindifference = \dimexpr\gre@dimen@begindifference + (\wd\gre@box@syllablenotes/2) - (\wd\gre@box@syllabletext/2) + \gre@dimen@temp@one - \gre@dimen@temp@two\relax%
    \gre@debugmsg{barspacing}{begindifference: \the\gre@dimen@begindifference}%
    %the end difference can be calculated from begindifference by decreasing begindifference by the width of the notes, increasing it by the width of the text, and then reversing the sign on the whole thing
    \gre@debugmsg{barspacing}{Correcting enddifference}%
    \gre@dimen@enddifference = \dimexpr-\gre@dimen@begindifference + \wd\gre@box@syllablenotes - \wd\gre@box@syllabletext\relax%
    \gre@debugmsg{barspacing}{enddifference: \the\gre@dimen@enddifference}%
    ...
    %we're going to start with the bar line, place the text relative to that, and then use the position of the bar line to reposition the cursor for the next syllable
    \ifdim\gre@dimen@temp@three < 0pt\relax%
      %we need to move back to where the previous notes ended
      \kern\gre@dimen@temp@three\relax%
    \fi%
    %now we move to the nominal start of the bar line
    \gre@hskip\gre@skip@temp@two%
    %move to the actual start of the bar line
    \kern\gre@dimen@temp@two%
    \kern\gre@dimen@temp@five%
    %print the bar line, saving the penalty for later
    ...
    \kern-\wd\gre@box@syllablenotes%
    \GreNoBreak% probably redundant, but better safe than sorry
    ...
    \ifgre@mustdotranslationcenterend%
      ...
      \gre@mustdotranslationcenterendfalse%
    \fi %
    %move back to the end of the notes%
    \kern\gre@dimen@enddifference%
    % if no end of line:
    \ifnum-1=\gre@newlinearg %
      %move to the nominal end of the notes%
      \kern-\gre@dimen@temp@two%
