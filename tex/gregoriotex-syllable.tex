...
\def\grecalculateglyphraisevalue#1#2{%
  ...
  \or% 12: if it is the brace above the bars, we shift it to a user-defined value
      \global\advance\greglyphraisevalue by -\grebraceshift %
  \fi%
...
\def\gresyllable#1#2#3#4#5#6#7#8#9{%
  ...
  \ifcase#4 %
    ...
    % we set gretempdim to the additional space that will be needed before the next syllable (just like it was somes lines above)
    \ifdim\greenddifference>0pt%
      \gretempdim=\greintersyllablespace %
    ...
    \ifdim\gretempdim<0pt %
      \gretempdim=0pt%
    \fi %
    % then we set gretempdim to the actual space between the texts of the two syllables
    \advance\gretempdim by \greenddifference %
    %\vrule width 2.7pt height 1mm%
    %
