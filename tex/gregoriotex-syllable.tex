...
\def\gre@endofsyllable#1#2#3{%
  ...
  \fi%
  \ifnum#1=1\relax %
    % if the next syllable is a bar, then it will take care of adding syllablefinalskip, if it isn't, then we add it ourselves
    \ifnum#3=1\relax %
      \GreNoBreak %
    \else%
...
\def\GreBarSyllable#1#2#3#4#5#6#7#8#9{%
  ...
  \gre@calculate@nextbegindifference{\gre@nextfirstsyllablepart}{\gre@nextmiddlesyllablepart}{\gre@nextendsyllablepart}{#7}%
  \gre@unsetfixednexttextformat %
  \gre@debugmsg{spacing}{previousenddifference: \the\gre@dimen@previousenddifference}%
  ...
  \gre@debugmsg{spacing}{nextbegindifference: \the\gre@skip@nextbegindifference}%
  \ifgre@newbarspacing%
    ...
    \gre@debugmsg{spacing}{calculate available space}%
    ...
    \fi%
    \gre@debugmsg{spacing}{space available to text: \the\gre@skip@temp@one}%
    \gre@debugmsg{spacing}{spacing available to notes: \the\gre@skip@temp@two}%
    ...
    \gre@debugmsg{spacing}{calculate required widths}%
    ...
    \fi%
    \gre@debugmsg{spacing}{text space after accounting for text: \the\gre@skip@temp@one}%
    \gre@debugmsg{spacing}{notes space after accounting for text: \the\gre@skip@temp@two}%
    ...
    \fi%
    \gre@debugmsg{spacing}{required space for text: \the\gre@skip@temp@one}%
    \gre@debugmsg{spacing}{required space for notes: \the\gre@skip@temp@two}%
    % now we divide the two distances in half to represent the position of the middle of each element
    \gre@debugmsg{spacing}{calculate position of middles}%
    \divide\gre@skip@temp@one by 2\relax%
    \divide\gre@skip@temp@two by 2\relax%
    \gre@debugmsg{spacing}{distance from previous text to middle of this text: \the\gre@skip@temp@one}%
    \gre@debugmsg{spacing}{distance from previous notes to middle of this notes: \the\gre@skip@temp@two}%
    % our final position calculation is to make sure that the centers of the text and the bar are not too far from each other
    \gre@debugmsg{spacing}{limit by offset}%
    ...
    \gre@debugmsg{spacing}{new begindifference approximation: \the\gre@dimen@begindifference}%
      \gre@debugmsg{spacing}{offset limit: \gre@space@dimen@maxbaroffset}%
    \ifdim\gre@dimen@begindifference > \gre@space@dimen@maxbaroffset\relax%
      ...
      \gre@debugmsg{spacing}{non-zero offsets}%
      \ifdim\gre@skip@temp@one > \gre@skip@temp@two\relax%
        \gre@debugmsg{spacing}{offset the text to the left}%
        \gre@dimen@temp@one = \dimexpr(\gre@dimen@begindifference - \gre@space@dimen@maxbaroffset)\relax%
        \gre@debugmsg{spacing}{check in}%
        \gre@dimen@temp@two = 0pt\relax%
        \gre@debugmsg{spacing}{check in 2}%
      \else%
        \gre@debugmsg{spacing}{offset the bar to the right}%
    ...
    \else\ifdim\gre@dimen@begindifference < -\gre@space@dimen@maxbaroffset\relax%
      ...
      \gre@debugmsg{spacing}{non-zero offsets}%
      \ifdim\gre@skip@temp@one > \gre@skip@temp@two\relax%
        \gre@debugmsg{spacing}{offset the text to the right}%
        \gre@dimen@temp@one = \dimexpr\gre@dimen@begindifference + \gre@space@dimen@maxbaroffset\relax%
        \gre@dimen@temp@two = 0pt\relax%
      \else%
        \gre@debugmsg{spacing}{offset the bar to the left}%
        \gre@dimen@temp@one = 0pt\relax%
        \gre@dimen@temp@two = \dimexpr-\gre@dimen@begindifference - \gre@space@dimen@maxbaroffset\relax%
      \fi%
    \else%
      %the two elements are sufficiently close together
      \gre@debugmsg{spacing}{zero offsets}%
      \gre@dimen@temp@one = 0pt\relax%
      \gre@dimen@temp@two = 0pt\relax%
    \fi\fi%
    ...
    \gre@debugmsg{spacing}{bar width: \the\wd\gre@box@syllablenotes}%
    \gre@debugmsg{spacing}{text width: \the\wd\gre@box@syllabletext}%
