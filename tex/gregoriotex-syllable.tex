...
\def\gre@calculate@simplenotesaligncenter#1{%
  \gre@trace{gre@calculate@simplenotesaligncenter{#1}}%
  \ifcase#1% 0: punctum
  ...
  \or% 1: flexus
    ...
    \gre@handleclivisspecialalignment{\gre@font@music\GreCPPesQuadratumLongqueueThreeNothing}{\gre@font@music\GreCPPunctum}%
  \or% 2: porrectus
  ...
  \or% 8: flexus with ambitus of one
    \gre@handleclivisspecialalignment{\gre@font@music\GreCPPesQuadratumLongqueueOneNothing}{\gre@font@music\GreCPPunctum}%
  \or% 9: flexus deminutus
    \gre@handleclivisspecialalignment{\gre@font@music\GreCPFlexusTwoDeminutus}{\gre@font@music\GreCPPunctum}%
  \or% 10: virgula
...
\def\gre@handleclivisspecialalignment#1#2{%
  \gre@trace{gre@handleclivisspecialalignment{#1}{#2}}%
...
\def\gre@calculate@notesaligncenter#1{%
  ...
  \gre@calculate@simplenotesaligncenter{#1}%
...
\def\GreSyllable#1#2#3#4#5#6#7#8#9{%
  ...
  \gre@calculate@enddifference{\wd\gre@box@syllablenotes}{\wd\gre@box@syllabletext}{\gre@dimen@textaligncenter}{\gre@dimen@notesaligncenter}%
  ...
  \ifgre@blockeolcustos\ifnum\gre@insidediscretionary=0\relax %
     \localrightbox{}%
  \fi\fi %
  \directlua{gregoriotex.save_post_syllable()}%
  \gre@trace@end%
}%
...
\def\gre@syllable@end#1#2#3#4{%
  ...
  \ifnum\gre@count@temp@one=1\relax %
    ...
    \else %
      % otherwise we call it with 1 only if there is no letters after
...
\def\GreBarSyllable#1#2#3#4#5#6#7#8#9{%
  ...
  \gre@beginningofscorefalse
  \gre@fixedtextformat{%
  \ifnum\gre@insidediscretionary=0
    \directlua{gregoriotex.save_syllable_info('bar')}%
  \else
    \directlua{gregoriotex.save_syllable_info('clefchange')}%
  \fi
  }%
  \gre@possibleluahyphenafterthissyllablefalse %
  \gre@showhyphenafterthissyllablefalse %
  ...
  % first of all we need to calculate begindifference and enddifference.
  #1%
  ...
  \gre@punctummoraadjustment % sets \gre@skip@punctummorashift and inserts kern
  \gre@debugmsg{barspacing}{Saving punctum mora shift: \the\gre@skip@punctummorashift}%
  \directlua{gregoriotex.save_min_distance('mora_shift', 'gre@skip@punctummorashift')}%
  \ifgre@newbarspacing%
    %
    % New bar spacing algorithm
    %
    \gre@skip@minTextDistance=0pt
    \ifdim\wd\gre@box@syllabletext > 0pt
      \ifnum#4=1
        \ifgre@in@euouae
          \gre@skip@minTextDistance=\gre@space@dimen@interwordspacetext@bars@euouae
        \else
          \gre@skip@minTextDistance=\gre@space@dimen@interwordspacetext@bars
        \fi
      \fi
    \else
      \ifnum#4=1
        \ifgre@in@euouae
          \gre@skip@minTextDistance=\gre@space@dimen@interwordspacetext@bars@notext@euouae
        \else
          \gre@skip@minTextDistance=\gre@space@dimen@interwordspacetext@bars@notext
        \fi
      \fi
    \fi
    \gre@debugmsg{barspacing}{Saving minimum text distance: \the\gre@skip@minTextDistance}%
    \directlua{gregoriotex.save_min_distance('text', 'gre@skip@minTextDistance')}%
    \ifdim\wd\gre@box@syllablenotes > 0pt
      % The box already has space built in, so don't add extra space
      \gre@skip@minNotesDistance=0pt
    \else
      % If there are no notes, add extra space
      \gre@skip@minNotesDistance=\gre@space@skip@interwordspacenotes
    \fi
    \gre@debugmsg{barspacing}{Saving minimum notes distance: \the\gre@skip@minNotesDistance}%
    \directlua{gregoriotex.save_min_distance('notes', 'gre@skip@minNotesDistance')}%
    \ifgre@textcleared%
      % Insert a zero kern that will be adjusted in the Lua function syllable_clearing.
      {\gre@attr@skip@type=5 \kern0pt}%
    \fi%
    \GreNoBreak
    \gre@hskip\glueexpr\gre@space@skip@bar@rubber
    % From left edge of syllable to left edge of text
    {\gre@attr@skip@type=2
    \ifdim\gre@dimen@begindifference>0pt
  ...
  \else%
    ...
    #8\relax
    \ifdim\wd\gre@box@syllabletext=0pt
      {\gre@attr@part=4\hbox{}}% don't print debugging marks
    \else
    ...
    \ifgre@shownotes
      #9%
    \else
      \hbox{}% ensure at least one node is marked as a note
    \fi
    \unsetattribute{\gre@attr@part}%
    {\gre@attr@skip@type=4
    \ifdim\gre@dimen@enddifference < 0pt\relax%
    ...
    \else
      \kern0pt
    \fi}%
    {\gre@attr@part=11\gre@penalty{\the\gre@space@count@endafterbarpenalty}}%
    \ifdim\wd\gre@box@syllabletext=0pt
      {\gre@attr@skip@type=1\gre@hskip0pt}%
    \else
      \ifnum\gre@lastoflinecount=1\relax
        \global\gre@lastoflinecount=2\relax
      \else
