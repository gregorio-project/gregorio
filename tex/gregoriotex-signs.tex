...
\def\gre@draw@curlybrace#1{%
  ...
  \gre@metapost{
    ...
    fill
      (  (0,0) % lower left
      -- (0.00651,0)
      .. controls (0.00977,0.01763) and (0.01506,0.03065) .. (0.02238,0.03906)
      .. controls ((0.08975*p)+0.02238,0.04801) and ((0.20032*p)+0.02238,0.05249) .. ((0.33173*p)+0.02238,0.05249)
      .. controls ((0.40225*p)+0.02238,0.05249) and ((0.51442*p)+0.02238,0.05086) .. ((0.66827*p)+0.02238,0.04761)
      .. controls ((0.79648*p)+0.02238,0.04408) and ((0.90705*p)+0.02238,0.04232) .. ((1*p)+0.02238,0.04232)
      .. controls (p+0.03567,0.04232) and (p+0.04829,0.04842) .. (p+0.06022,0.06063)
      .. controls (p+0.07189,0.07365) and (p+0.0807,0.0906) .. (p+0.08667,0.11149) % lower beak
      .. controls (p+0.09264,0.0906) and (p+0.10145,0.07365) .. (p+0.11312,0.06063)
      .. controls (p+0.12505,0.04842) and (p+0.13767,0.04232) .. ((1*p)+0.15096,0.04232)
      .. controls ((1.09295*p)+0.15096,0.04232) and ((1.20352*p)+0.15096,0.04408) .. ((1.33173*p)+0.15096,0.04761)
      .. controls ((1.48558*p)+0.15096,0.05086) and ((1.59775*p)+0.15096,0.05249) .. ((1.66827*p)+0.15096,0.05249)
      .. controls ((1.79968*p)+0.15096,0.05249) and ((1.91025*p)+0.15096,0.04801) .. ((2*p)+0.15096,0.03906)
      .. controls ((2*p)+0.15828,0.03065) and ((2*p)+0.16357,0.01763) .. ((2*p)+0.16683,0)
      -- ((2*p)+0.16683,0) -- ((2*q)+0.13509,0) -- ((2*q)+0.13509,0) % lower right
      .. controls ((2*q)+0.13238,0.02658) and ((2*q)+0.12424,0.04774) .. ((2*q)+0.11068,0.06348)
      .. controls ((1.87189*q)+0.11068,0.07975) and ((1.71111*q)+0.11068,0.08789) .. ((1.51765*q)+0.11068,0.08789)
      .. controls ((1.45229*q)+0.11068,0.08789) and ((1.36863*q)+0.11068,0.08626) .. ((1.26667*q)+0.11068,0.08301)
      .. controls ((1.14902*q)+0.11068,0.07894) and ((1.06013*q)+0.11068,0.0769) .. ((1*q)+0.11068,0.0769)
      .. controls (q+0.0866,0.0769) and (q+0.07454,0.10644) .. (q+0.07121,0.13102)
      -- (q+0.07121,0.13102) -- (q+0.06388,0.13102) -- (q+0.06388,0.13102) % upper beak
      .. controls (q+0.06055,0.10644) and (q+0.04849,0.0769) .. ((1*q)+0.02441,0.0769)
      .. controls ((0.93987*q)+0.02441,0.0769) and ((0.85098*q)+0.02441,0.07894) .. ((0.73333*q)+0.02441,0.08301)
      .. controls ((0.63137*q)+0.02441,0.08626) and ((0.54771*q)+0.02441,0.08789) .. ((0.48235*q)+0.02441,0.08789)
      .. controls ((0.28889*q)+0.02441,0.08789) and ((0.12811*q)+0.02441,0.07975) .. ((0*q)+0.02441,0.06348)
      .. controls (0.01085,0.04774) and  (0.00271,0.02658) ..  (0,0)
      -- cycle
      ) transformed t;
    setbounds currentpicture to
      ( (0,-0.67980) -- ((2*q)+0.13509,-0.67980) -- ((2*q)+0.13509,0.13102) -- (0,0.13102) -- cycle )
      transformed t;
    endfig;
  }%
}%
...
\gdef\gre@barshiftaftermora{3}

\def\gresetbarshiftaftermora#1{%
  \IfStrEq{#1}{always}%
    {\gdef\gre@barshiftaftermora{3}}%
    {\IfStrEq{#1}{notextonly}%
      {\gdef\gre@barshiftaftermora{2}}%
      {\IfStrEq{#1}{insideonly}%
        {\gdef\gre@barshiftaftermora{1}}%
        {\IfStrEq{#1}{never}%
          {\gdef\gre@barshiftaftermora{0}}%
          {\gre@error{Unrecognized option "#1" in gresetbarshiftaftermora , use 'always', 'notextonly', 'insideonly' or 'never'}}%
        }%
      }%
    }%
}%

\def\gre@kern@bar@aftermora{%
  \setbox\gre@box@temp@width=\hbox{\gre@font@music\gre@fontchar@punctummora}%
  \kern\glueexpr -\wd\gre@box@temp@width - \gre@space@skip@spacebeforesigns + \gre@space@skip@barmoraadjustment\relax %
}
...
\def\gre@writebar#1#2#3#4{%
  ...
  \ifgre@lastispunctum%
    \ifcase\gre@barshiftaftermora\or %
      \ifcase #2\or %
        \gre@kern@bar@aftermora %
      \fi %
    \or %
      \ifcase #3\relax %
        \gre@kern@bar@aftermora %
