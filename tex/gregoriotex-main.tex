...
\def\gre@annotation[#1]#2{%
  ...
  \else%
    ...
    \ifgre@annotationbottomline%
      \ifcase\gre@count@annotationvalign\relax%
        %case 0 = top of first line
        \global\gre@dimen@annotationtrueraise=\dimexpr\ht\gre@box@annotation@old+\dp\gre@box@annotation@old+\gre@dimen@annotationseparation-\ht\gre@box@annotation@add\relax%
        \gre@debugmsg{annotation}{Aligning to top of last line: \the\gre@dimen@annotationtrueraise}%
      \or %case 1 = baseline of first line
        \global\gre@dimen@annotationtrueraise=\dimexpr\ht\gre@box@annotation@old+\dp\gre@box@annotation@old+\gre@dimen@annotationseparation\relax%
        \gre@debugmsg{annotation}{Aligning to baseline of last line: \the\gre@dimen@annotationtrueraise}%
      \or %case 2 = bottom of first line
        \global\gre@dimen@annotationtrueraise=\dimexpr\ht\gre@box@annotation@old+\dp\gre@box@annotation@old+\gre@dimen@annotationseparation+\dp\gre@box@annotation@add\relax%
...
\def\gresetmodenumbersystem#1{%
  \IfStrEq{#1}{roman-majuscule}%
    {\let\GreModeNumber\gre@romannumeral@majuscule}%
    {\IfStrEq{#1}{roman-minuscule}%
      {\let\GreModeNumber\gre@romannumeral@minuscule}%
      {\IfStrEq{#1}{arabic}%
        {\let\GreModeNumber\relax}%
        {\gre@error{Unrecognized option in \protect\gresetmodenumbersystem}}%
      }%
    }%
}%
...
\def\grechangeglyph#1#2#3{%
  ...
  \directlua{gregoriotex.init_variant_font([[#2]],true, [[\the\gre@factor]], [[\the\gre@count@temp@two]])}%
...
{%
  \directlua{gregoriotex.init_variant_font('greciliae', true, [[\the\gre@factor]], '100000')}%
...
\def\gre@setgregoriofontscaled[#1]#2#3{%
  ...
  \else %
    \directlua{texio.write_nl('test1')texio.write_nl('test1')}%
...
\def\gre@loadgregoriofont{%
  ...
  {\gre@font@music\directlua{gregoriotex.check_font_version() gregoriotex.scale_score_fonts([[\the\gre@count@temp@three]], [[\the\gre@count@temp@two]], [[\the\gre@factor]])}}%
  \relax%
}%
