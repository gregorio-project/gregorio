...
\def\gre@trace#1{%
  \IfSubStr{\gre@debug}{,trace,}%
    {%print tracing messages and increase indent
      \gre@typeout{\gre@trace@prefix #1}%
...
\xdef\gre@gregoriotexversion{5.0.0-beta2}% GREGORIO_VERSION - VersionManager.py
...
\def\gre@widthof#1{%
  % we don't trace this function's arguments because they often don't take
  % well to appearing in the log when expanded and aren't useful when
  % unexpanded
  \gre@trace{gre@widthof}%
