...
\def\gre@setinitial#1{%
  ...
  \else % a multi-line initial
    %account for fixed height elements of a line
    \gre@dimen@temp@one = \dimexpr(- \gre@space@dimen@spacebeneathtext %
        - \gre@space@dimen@spacelinestext %
        - \gre@dimen@staffheight%
        - \gre@space@dimen@spaceabovelines%
        )\relax%
    \advance\gre@dimen@temp@five by \dimexpr(\gre@dimen@temp@one * (\gre@count@initiallines - 1))\relax%%adjust the initial position based on the number of lines it’s supposed to take up
    %account for the variable height elements of each line in initial
    {% initiating a group so that the distance changes we're about to make won't persist beyond the calculation of where the initial goes
    ...
    \global\setbox\gre@box@initial=\hbox{\vtop to 0pt{\hbox to \gre@dimen@initialwidth {\hss\raise \gre@dimen@temp@five\box\gre@box@initial\hss}\vss}}%
  \fi%
  }%
