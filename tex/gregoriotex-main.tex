...
\def\GreNewLineWithSpace#1#2#3#4{%
  \gre@debugmsg{lineheight}{GreNewLineWithSpace #1 #2}%
  \grenewlinecommon{#1}{#2}{#3}{0}{#4}%
  \relax%
}%

% basically same as above, but these one do a \hfill, the lines are not justified
%macro to call if you want to go to the next line simply
\def\GreNewParLineWithSpace#1#2#3#4{%
  \gre@debugmsg{lineheight}{GreNewParLineWithSpace #1 #2}%
...
\def\grenewlinecommon#1#2#3#4#5{%
  ...
  \ifnum\greboxing=0\relax %
    ...
    \ifcase\gre@variableheightexpansion %
      \gre@calculate@additionalspaces{#1}{#2}% 0
      % else it's handled by the variable height code
    \fi
    \ifnum#3 = 1\relax %
...
\def\GreBeginScore#1#2#3{%
  ...
  \noindent%
  \directlua{
    gregoriotex.atScoreBeginning([[#1]], #2, #3,
...
\def\GreEndScore{%
  ...
  \grelocalleftbox{}%
  \grelocalrightbox{}%
  \gre@calculate@additionalspaces{\gre@pitch@g}{\gre@pitch@g}%
  \global\gre@dimen@currentabovelinestextheight=0 sp%
  \gre@removetranslationspace %
  \grenormalinitial %
  \grerestorepenalties %
...
\AtBeginDocument{\opengreaux\IfStrEq{\gre@debug}{}{}{\typeout{GregorioTeX is in debug mode}\typeout{\gre@debug\space messages will be printed to the log.}}}%
