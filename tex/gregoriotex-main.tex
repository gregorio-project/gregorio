  ...
  \ifgre@boxing\else%
    \IfStrEqCase{\gre@initialend}{%
      ...
      {semi-manual}%
        ...
        \gre@bug{Invalid value for \protect\gresetinitialend}%
      ]%
...
\def\gresetinitialend#1{%
  \IfStrEqCase{#1}{
    {manual}%
      {\gdef\gre@initialend{manual}}%
    {semi-manual}%
      {\gdef\gre@initialend{semi-manual}}%
    }[% else
...
\def\gre@adjustsecondline{%
  ...
  %we provide for a special exemption here for 1-line initials so that the most common case can be completely automatic
...
\def\gre@setinitial#1{%
  ...
  \gre@dimen@temp@one = \dimexpr(-\gre@dimen@additionalbottomspace %
      - \gre@space@dimen@spacebeneathtext %
      - \gre@space@dimen@spacelinestext %
      - \gre@dimen@staffheight%
      - \gre@dimen@currenttranslationheight %
      - \f@size pt)\relax%
  %adjust the initial position based on the number of lines it’s supposed to take up
  \advance\gre@dimen@temp@five by \dimexpr(\gre@dimen@temp@one * (\gre@count@initiallines - 1))\relax%
