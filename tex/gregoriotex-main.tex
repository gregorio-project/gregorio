...
\def\GreFinalNewLine{%
  \gre@debugmsg{knownline}{Extra end of score increment to knownline}
  \global\advance\gre@count@knownline by 1\relax %
}%

% the macro we call each time we force a changing of line, it automatically sets \gre@count@knownline, and adjusts left spaces
...
\def\gre@newlinecommon#1#2{%
  ...
  \fi\fi %
  \ifgre@boxing\else%
    \ifgre@manualinitialend%
      \ifnum\gre@count@initiallines=\gre@count@knownline %
        \gre@adjustlinesafterinitial %
      \fi%
    \else%
      \gre@adjustlinesafterinitial%
    \fi
    \gre@debugmsg{knownline}{I’m manually ending line \the\gre@count@knownline}%
    \global\advance\gre@count@knownline by 1\relax %
...
%setting for how many lines of score wrap the initial
%applies when initialend is manual or auto
\newcount\gre@count@initiallines%
\gre@count@initiallines=1\relax% default to one-line initials
\def\gresetinitiallines#1{%
  \IfInteger{#1}%
    {%
      \ifnum#1>-1\relax%
        \gre@count@initiallines=#1%
      \else%
...
\newif\ifgre@manualinitialend%

\def\gresetinitialend#1{%
  \IfStrEqCase{#1}{
    {manual}%
      {\gre@manualinitialendtrue}%
    {semi-manual}%
      {\gre@manualinitialendfalse}%
    }[% else
      \gre@error{Unrecognized option "#1" for \protect\gresetinitialend\MessageBreak Possible options are: 'manual' and 'semi-manual'}%
    ]%
}
\gresetinitialend{manual}% manual is the legacy mode and thus the default


%% box containing the initial, and dimen containing its width (and the width of the space after)
\newbox\gre@box@initial%

% gre@count@knownline is the line we think we are in
\newcount\gre@count@knownline%
...
\def\gre@adjustsecondline{%
  ...
  %we provide for a special exemption here for 1-line initials so that the most common case can be completely automatically
    \gre@dimen@additionalleftspace=0pt\relax%
  \else%
...
\def\gre@adjustlinesafterinitial{%
  \gre@trace{gre@adjustlinesafterinitial}%
...
\def\gre@setinitial#1{%
  ...
  \ifvoid\gre@box@initial%
    \gre@debugmsg{initial}{Fill initial box with #1}%
  ...
  \ifnum\gre@count@initiallines=1\relax % a one line initial
    \setbox\gre@box@initial=\hbox to \gre@dimen@initialwidth {\hss\raise \gre@dimen@temp@five\box\gre@box@initial\hss}%
  \else % a multi-line initial
    %fixed height elements of a line will form the basis of each line’s adjustment to the initial position
    \gre@dimen@temp@one = \dimexpr(\gre@space@dimen@spacebeneathtext %
        + \gre@space@dimen@spacelinestext %
        + \gre@dimen@staffheight%
        + \gre@space@skip@spaceabovelines	%
        )\relax%
    \gre@debugmsg{initial}{Fixed elements line height: \the\gre@dimen@temp@one}%
    %account for the variable height elements of each line in initial
    \begingroup% initiating a group so that the distance changes we're about to make won't persist beyond the calculation of where the initial goes
      %first set the base "unit" distance
      \ifgre@noteadditionalspacelinestext%
        \gre@dimen@additionalbottomspace=\gre@space@dimen@noteadditionalspacelinestext\relax%
      \else%
        \gre@dimen@additionalbottomspace=\dimexpr(((\gre@dimen@interstafflinedistancebase + \gre@dimen@stafflinethicknessbase) / 2 ) * \gre@factor)\relax%
      \fi%
      \gre@dimen@currenttranslationheight=\gre@space@dimen@translationheight\relax%
      \gre@dimen@currentabovelinestextheight=\gre@space@dimen@abovelinestextheight\relax
      %now switch to lua to access the line heights information and make the adjustment
      \directlua{gregoriotex.adjust_initial_raise(\gre@pitch@cleftop,\gre@pitch@clefbottom,\gre@pitch@adjust@top,\gre@pitch@adjust@bottom)}%
      \gre@debugmsg{initial}{Final initial raise: \the\gre@dimen@temp@five}%
      \global\setbox\gre@box@initial=\hbox{\vtop to 0pt{\hbox to \gre@dimen@initialwidth {\hss\raise%
        \gre@dimen@temp@five%
        \box\gre@box@initial\hss}\vss}}%
    \endgroup%
  \fi%
...
\def\GreBeginScore#1#2#3#4#5#6#7#8{%
  ...
  \fi %
  \gre@debugmsg{knownline}{Initialize knownline as 1}%
  \global\gre@count@knownline=1\relax %
  \global\gre@lastoflinecount=2\relax %
...
\def\GreEndScore{%
  \ifgre@manualinitialend%
    \ifnum\gre@count@initiallines>1\relax
      \ifnum\gre@count@initiallines>\numexpr(\gre@count@knownline-1)\relax%
        \gre@error{Score ended before\MessageBreak finding the end of the lines wrapping the initial.\MessageBreak  Make sure you have at least \the\gre@count@initiallines\space manual line breaks\MessageBreak in your gabc}%
      \fi%
    \fi%
  \else%
    \ifnum\gre@count@initiallines>1\relax%
      \ifnum\gre@count@knownline<2\relax%
        \gre@error{Score ended before\MessageBreak finding the end of the lines wrapping the initial.\MessageBreak Make sure you have at least 1 manual line break\MessageBreak in your gabc}%
      \fi%
    \fi%
  \fi%
