...
\def\GreSetStackedSyllable#1#2#3#4#5#6#7#8{%
...
% \GreSetStackedSyllable, the same way \GreWriteStackedLyric used to do it
% directly: centers it under level 1's alignment center
% (\gre@dimen@textaligncenter, read-only from here on -- see
% \gre@measurecenter) and raises it by lyricstackseparation per level.
% #1: level (2 for the first additional line)
\def\gre@buildstackedlyriclevel#1{%
  \ifgre@showlyrics%
    \ifcase\csname gre@stackedlyric@level@#1@forced\endcsname\relax% no forced center: honor the centering mode
      \gre@selectcenteringparts
    ...
    \else% forced center
      \gdef\gre@stackedlyric@firstpart{\csname gre@stackedlyric@level@#1@pre\endcsname}%
      \gdef\gre@stackedlyric@middlepart{\csname gre@stackedlyric@level@#1@mid\endcsname}%
      \gdef\gre@stackedlyric@endpart{\csname gre@stackedlyric@level@#1@post\endcsname}%
    \fi%
    % this level's alignment center, as in \gre@calculate@textaligncenter
    % (unwrapped: the fixed-style optimization only applies to level 1)
    \gre@measurecenter{\gre@stackedlyric@firstpart\gre@stackedlyric@middlepart}%
      {\gre@stackedlyric@middlepart}{\gre@dimen@stackedlyriccenter}%
    ...
    \directlua{gregoriotex.set_lyric_line_dash(#1, \csname gre@stackedlyric@level@#1@eow\endcsname)}%
    % \gre@attr@part scoping: see \GreWriteStackedLyric below.
    {\gre@attr@part=\numexpr9+#1\relax%
    ...
    \hbox attr \gre@attrid@part=\numexpr9+#1\relax to 0pt{%
      ...
      \hbox{\gre@stackedlyric@firstpart\gre@stackedlyric@middlepart\gre@stackedlyric@endpart}%
      \hss}}%
  \fi%
}%

% Writes one additional lyric line (level 2+) of a stacked syllable
% immediately, without going through the stash-and-defer mechanism above.
% Kept only for syllable types other than \GreSyllable (see
% write_extra_lyric_lines in gregoriotex-write.c): those don't call
% \gre@dostackedlyriclevels, so \GreSetStackedSyllable's stashed data would
% never get typeset for them.
% #1: level (2 for the first additional line)
% #2: end of word: 1 if this level ends a word here, else 0
% #3: forced center: 1 if the level has a {} forced center, else 0
% #4: the first letters, they don't count for the alignment
...
% #7: the very first letter (for first-letter centering)
% #8: all the letters after the first letter
\def\GreWriteStackedLyric#1#2#3#4#5#6#7#8{%
  \ifgre@showlyrics%
    \ifcase#3\relax% no forced center: honor the centering mode
      \gre@selectcenteringparts{#4}{#5}{#6}{#7}{#8}%
        {\gre@stackedlyric@firstpart}{\gre@stackedlyric@middlepart}{\gre@stackedlyric@endpart}%
    \else% forced center
      \def\gre@stackedlyric@firstpart{#4}%
      \def\gre@stackedlyric@middlepart{#5}%
      \def\gre@stackedlyric@endpart{#6}%
    \fi%
    % this level's alignment center, as in \gre@calculate@textaligncenter
    % (unwrapped: the fixed-style optimization only applies to level 1)
    \gre@measurecenter{\gre@stackedlyric@firstpart\gre@stackedlyric@middlepart}%
