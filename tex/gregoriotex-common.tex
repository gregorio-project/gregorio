...
\gre@declarefileversion{gregoriotex-common.tex}{6.1.0-beta1}% GREGORIO_VERSION
...
\newcount\gre@parsecommas@cur
\def\gre@parsecommas#1#2{%
  % https://tex.stackexchange.com/a/510410
  \def\gre@options{,#1,}%
  \gre@debugmsg{parsecommas}{\gre@options}%
  \IfStrEq{\gre@options}{,,}{%
    % do nothing
  }{%
    \StrCount{\gre@options}{,}[\gre@total]% number of items plus one
    \gre@parsecommas@cur=1\relax
    \loop\ifnum\gre@parsecommas@cur<\gre@total\relax
      \StrBetween[\the\gre@parsecommas@cur,\the\numexpr\gre@parsecommas@cur+1]{\gre@options}{,}{,}[\gre@option]%
      #2{\gre@option}%
      \advance\gre@parsecommas@cur by 1\relax
    \repeat
  }%
}
