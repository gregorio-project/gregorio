...
\def\greinitial#1{%
  ...
  % we print the initial always at the same place, and the we print the Aboveinitialfirstbox centered
  ...
  \ifdim\wd\Aboveinitialfirstbox=0 pt\relax %
  \else %
    \temp=\initialwidth %
    \advance\temp by -\wd\Aboveinitialfirstbox %
    \divide\temp by 2 %
    \hskip -\initialwidth %
    \kern\temp %
    \raise\aboveinitialfirstraise\copy\Aboveinitialfirstbox %
    \kern\temp %
    \setbox\Aboveinitialfirstbox=\hbox{}%
  \fi %
  % then we center the second box above the initial, if there is one
  \ifdim\wd\Aboveinitialsecondbox=0 pt\relax %
  \else %
    \temp=\initialwidth %
    \advance\temp by -\wd\Aboveinitialsecondbox %
    \divide\temp by 2 %
    \hskip -\initialwidth %
    \kern\temp %
    \raise\aboveinitialsecondraise\copy\Aboveinitialsecondbox %
    \kern\temp %
    \setbox\Aboveinitialsecondbox=\hbox{}%
  \fi %
  \relax %
}
...
\newbox\Aboveinitialfirstbox 
\newdimen\aboveinitialfirstraise
\newbox\Aboveinitialsecondbox 
\newdimen\aboveinitialsecondraise
% the height difference between the bottom of the first annotation line and the top of the second annotation line (above the initial) is controlled by the \aboveinitialseparation space, in gregoriotex-spaces.tex
...
\def\setfirstlineaboveinitial#1#2{%
  ...
  % here the variables are a bit strange... we can calculate the final value of aboveinitialfirstraise only after the beginning of the score, after the call to this macro. So we just set aboveinitialfirstraise to the negative value the #2 will add, and then, after the beginning of the score, we'll add setaboveinitialraise that will... set aboveinitialfirstraise.
  \setbox\Aboveinitialfirstbox=\hbox{#2}%
  \global\aboveinitialfirstraise=-\ht\Aboveinitialfirstbox %
  \global\setbox\Aboveinitialfirstbox=\hbox{#1}%
  \relax %
}

\def\setfirstannotation#1{%
...
\def\setsecondannotation#1{%
  ...
  \global\aboveinitialsecondraise=-\ht\Aboveinitialsecondbox %
...
\def\setaboveinitialraise{%
  \global\advance\aboveinitialfirstraise by \staffheight %
  \global\advance\aboveinitialfirstraise by \spacebeneathtext %
  \global\advance\aboveinitialfirstraise by \translationheight %
  \global\advance\aboveinitialfirstraise by \spacelinestext %
  \global\advance\aboveinitialfirstraise by \additionalbottomspace %
  \global\advance\aboveinitialsecondraise by \aboveinitialfirstraise %
  \relax %
}
...
\def\abovelinestextstyle#1{%
\small\itshape #1\relax %
}
...
\edef\disableeolshifts{0}

\def\greDisableEOLShifts{%
  \xdef\disableeolshifts{1}%
}

\def\greEnableEOLShifts{%
  \xdef\disableeolshifts{0}%
}
