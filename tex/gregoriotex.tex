...
\def\gredeclarefileversion#1#2{%
  \ifnum\number#2=\number\greinternalversion\else %
    \greerror{uncoherent file versions: gregoriotex.tex is version \number\greinternalversion \space\space while #1 is version \number#2}
  \fi %
}
...
\def\gregoriotexapiversion#1{%
  \ifnum\number#1=\number\gregoriotexversion\else %
    \greerror{GregorioTeX is version \number\gregoriotexversion\ while a score you included requires version \number #1. Please recompile your scores}
  \fi %
}
