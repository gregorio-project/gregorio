...
\raise\glyphraisevalue\hbox{\gregoria ^^^^03e9}
\else % we consider that it is a f key
\raise\glyphraisevalue\hbox{\gregoria ^^^^03ea}
\fi%
}
...
\setbox\Tempwidth=\hbox{\gregoria #1}%
\tempwidth=\wd\Tempwidth%
\raise \glyphraisevalue%
\copy\Tempwidth%
\setcusto{#3}%
...
\raise\glyphraisevalue\hbox{\gregoria ^^^^03f0}%
\hskip\barspace %
\penalty 0% test
}
...
\hskip\barspace %
\calculateglyphraisevalue{8}% bar glyphs are made to be at this height
\raise\glyphraisevalue\hbox{\gregoria ^^^^03f1}%
\hskip\barspace %
}
...
\hskip\barspace %
\calculateglyphraisevalue{8}% bar glyphs are made to be at this height
\raise\glyphraisevalue\hbox{\gregoria ^^^^03f2}%
\hskip\barspace %
}
...
\hskip\barspace %
\calculateglyphraisevalue{8}% bar glyphs are made to be at this height
\raise\glyphraisevalue\hbox{\gregoria ^^^^03f3}%
\hskip\barspace %
}
...
% we open the font gregoria at \factor pt, we must know \factor because it will determine the line heigth, etc.
\font\gregoria=gregoria at \the\factor em
