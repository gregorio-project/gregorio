...
\def\grechangedim#1#2#3{%
  ...
  \ifcsname gre@space@\gre@prefix @#1\endcsname%
    ...
    \gre@dimension{#1}{#2}{#3}%
    %If we are calling grechangedim from a space configuration file and the space 
                 %configuration file is designed for a \gre@factor other than the current one, then we 
                 %need to rescale the distance being changed.
