...
\def\gresetstafflinefactor#1{%
  \gre@error{\protect\gresetstafflinefactor\space is obsolete.\MessageBreak Use \protect\grechangestafflinethickness\space instead.}%
}%
  ...
  \ifdim#1 <0pt%
    ...
    \ifgre@eolshiftsenabled %
      \ifgre@blockeolcustos\else%
        \setbox\gre@box@temp@width=\hbox{\gre@pickcusto{\gre@pitch@g}}%
...
\def\gre@calculate@textaligncenter#1#2#3{%
  \ifnum#3=0\relax%
    \grewidthof{\gre@fixedtextformat{#1#2}}%
  \else %
    \grewidthof{\gre@fixednexttextformat{#1#2}}%
  \fi %
  \global\gre@dimen@textaligncenter=\the\gre@dimen@temp@three %
  \ifnum#3=0\relax%
    \grewidthof{\gre@fixedtextformat{#2}}%
  \else %
    \grewidthof{\gre@fixednexttextformat{#2}}%
  \fi %
...
\def\GreSetAboveInitialSeparation#1{%
  \gre@error{\protect\GreSetAboveInitialSeparation\space is obsolete.\MessageBreak Use \protect\grechangedim\{annotationseparation\}\space instead.}%
}%

%To change the space after the initial
%First argument is distance, second is whether it should scale when \gre@factor changes.
\ifdefined\setspaceafterinitial%
  \gre@error{\protect\setspaceafterinitial\space is already defined.  Check for package conflicts.}%
\else%
...
\ifdefined\setspacebeforeinitial%
  \gre@error{\protect\setspacebeforeinitial\space is already defined.  Check for package conflicts.}%
\else%
...
\ifdefined\setinitialspacing%
  \gre@error{\protect\setinitialspacing\space is already defined.  Check for package conflicts.}%
\else%
