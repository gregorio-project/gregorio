...
\def\gre@calculate@glyphraisevalue#1#2{%
  ...
  \ifcase\greisonaline\or% isonaline = 1
    \ifcase#2 %
    \or% 1
      \global\advance\gre@temp@count@three by 1%
    \or% 2
      \global\advance\gre@temp@count@three by -1%
    \or% 3
      \global\advance\gre@temp@count@three by -1%
    ...
    \fi%
  \fi%
  \global\advance\gre@temp@count@three by -7 %
  \global\gre@glyphraisevalue = 15750 sp %
  \global\multiply\gre@glyphraisevalue by \the\grefactor %
  \global\multiply\gre@glyphraisevalue by \the\gre@temp@count@three %
  ...
  \or% 10: if it is a low choral sign, we shift it a bit lower, of a user-defined value
    \grelowchoralsignstyle{\global\advance\gre@glyphraisevalue by -\gre@lowchoralsignshift} %
  \or% 11: if it is a high choral sign, we shift it a bit lower, of a user-defined value
    \ifnum\greisonaline=1%
      \grehighchoralsignstyle{\global\advance\gre@glyphraisevalue by -\gre@highchoralsignshift} %
    \else %
      \grelowchoralsignstyle{\global\advance\gre@glyphraisevalue by -\gre@lowchoralsignshift} %
    \fi %
  ...
  \or% 12: if it is the brace above the bars, we shift it to a user-defined value
      \global\advance\gre@glyphraisevalue by -\gre@braceshift %
  \fi%
...
\def\gre@calculate@textaligncenter#1#2#3{%
  ...
  \fi %
  \global\gre@textaligncenter=\the\gre@temp@dimen@three %
  \ifnum#3=0\relax%
  ...
  \global\advance\gre@textaligncenter by -\the\gre@temp@dimen@three%
  \relax%
}%
...
\newdimen\gre@temp@dimen@two%
\newdimen\gre@temp@dimen@three%
\newdimen\gre@temp@dimen@four%
\newdimen\gre@temp@dimen@five%

\newskip\gre@temp@skip@one%
\newskip\gre@temp@skip@two %
\newskip\gre@temp@skip@three%
\newskip\gre@temp@skip@four%

\newcount\gre@temp@count@one%
\newcount\gre@temp@count@two%
\newcount\gre@temp@count@three%
...
\def\gre@changeonedimenfactor#1#2#3{%
	...
	\ifrubber% if we have a rubber allowed length we create a temporary skip
		\let\gre@scaledist\gre@temp@skip@one%
	\else% otherwise we create a temporary dimen
		\let\gre@scaledist\gre@temp@dimen@one%
	\fi%
	% Math
...
\def\gre@convertto#1#2{%
	...
	\else%
		...
		\fi%
		\gre@temp@count@one = \expandafter\gre@makein\the\gre@unit%
		\divide\gre@base by \gre@temp@count@one%
		\edef\gre@converted{%
...
\def\gre@changedimenfactor#1#2{%
	...
	\ifnum\gre@scale@stafflinefactor=1\relax%
		\gre@temp@count@two = \gre@stafflinefactor%
		\multiply\gre@temp@count@two by #2\relax%
		\divide\gre@temp@count@two by #1\relax%
		\xdef\gre@stafflinefactor{\the\gre@temp@count@two}%
	\fi%
	\gre@computespaces%
	\relax %
}%
