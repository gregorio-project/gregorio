...
char
libgregorio_opustex_is_out_of_neume (gregorio_element * element)
{
  if (!element)
    {
      return 0;
				    ...
				    char first_syllable)
{
  ...
  if (libgregorio_opustex_is_out_of_neume (current_element))
    {
      ...
      if (current_element->type == GRE_END_OF_LINE)
	{
	  if (syllable->next_syllable)
	    {
	      if (syllable->next_syllable->elements[0]->first_glyph->
		  first_note->pitch)
		{
		  fprintf (f, "\\custos ");
		  libgregorio_opustex_print_note (f,
						  syllable->next_syllable->
						  elements[0]->first_glyph->
						  first_note->pitch);
		  fprintf (f, "\n");
		}
	      else
		{
		  ...
		  gregorio_glyph *next_glyph =
		    syllable->next_syllable->elements[0]->next_element->
		    first_glyph;
		  while (next_note == 0)
		    {
		      if (next_glyph && next_glyph->first_note->pitch)
			{
			  next_note = next_glyph->first_note->pitch;
			  fprintf (f, "\\custos ");
...
{
  int printed = 0;
  if (c < 128 && c != 35 && c != 36 && c != 37 && c != 38 && c != 95
