...
libgregorio_calculate_new_key (char step, int line)
{
  switch (step)
    ...
    case F_KEY:
      ...
      break;
    default:
      libgregorio_message (_("can't calculate key"),
...
libgregorio_det_step_and_line_from_key (int key, char *step, int *line)
{
  switch (key)
    ...
    default:
      *step = 0;
      *line = 0;
      libgregorio_message (_("can't determine step and line of the key"),
...
libgregorio_det_pitch (int key, char step, int octave)
{
  switch (octave)
    ...
    case (3):
      ...
      break;
    default:
      libgregorio_message (_("unknown octave"),	//TODO : à améliorer
  ...
  for (i = 0; i < score->number_of_voices; i++)
      ...
      if (element->type == GRE_C_KEY_CHANGE)
	{
	  ...
	  snprintf (error, 80,
		    ...
		    i + 1);
	  libgregorio_message (error, "libgregorio_fix_initial_keys", VERBOSE,
			       0);
