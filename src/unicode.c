...
This file contains some functions to handle UTF-8 correctly. We chose to embed
them instead of relying on locale or glib, for simplification: with these
functions we don't rely on anything locale-dependant (such as wchar_t). The
reason is that Windows XP (...) is not UTF-8 and thus would need the glib in
...
void
gregorio_print_unichar (FILE * f, grewchar to_print)
{
  if (to_print <= 0x7F)
    {
...
void
gregorio_print_unistring (FILE * f, grewchar * first_char)
{
  while (*first_char != 0)
    {
