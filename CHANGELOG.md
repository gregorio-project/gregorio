...
- Fixed vertical spacing so that rare signs (accentus, circulus, semicirculus, musica ficta) no longer collide with above-lines text, commentary, or the above-lines nabc line, and so that the below-lines nabc line no longer collides with the low note it sits under. `additionaltopspacenabcthreshold` now reacts as soon as content reaches the top of the staff, instead of leaving a gap where above-lines nabc never grew to make room; this also fixes the same collision for plain high notes with no rare sign involved. `abovelinesnabcraise` was also increased slightly, since the accentus glyph's tip reaches a bit higher than the pitch-step-based reservation accounted for. The new count `additionalbottomspacenabcthreshold` lets below-lines nabc react to low notes independently of the lyrics threshold. See [#1766](https://github.com/gregorio-project/gregorio/issues/1766).
