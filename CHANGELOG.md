...
- Fixed vertical spacing so that rare signs (accentus, circulus, semicirculus, musica ficta) no longer collide with above-lines text, commentary, or the above-lines nabc line, and so that the below-lines nabc line no longer collides with the low note it sits under. The new count `additionalbottomspacenabcthreshold` lets below-lines nabc react to low notes independently of the lyrics threshold. See [#1766](https://github.com/gregorio-project/gregorio/issues/1766).
