...
- Fixed vertical spacing so that rare signs (accentus, circulus, semicirculus, musica ficta) no longer collide with above-lines text, commentary, or the above-lines nabc line, and so that below-lines nabc no longer collides with the low note it sits under. Above-lines nabc now grows as soon as content reaches the top of the staff and gained a bit more headroom above it, fixing the same collision for plain high notes too. The new count `additionalbottomspacenabcthreshold` gives below-lines nabc the same reactive behavior, independently of the lyrics threshold. See [#1766](https://github.com/gregorio-project/gregorio/issues/1766).
