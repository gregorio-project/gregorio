...
- Fixed vertical spacing so that below-lines nabc no longer collides with the low note it sits under; the new count `additionalbottomspacenabcthreshold` gives it the same reactive behavior that the lyrics already had, independently of the lyrics threshold. Above-lines nabc now also grows as soon as content reaches the top of the staff, instead of only for unusually tall content. See [#1766](https://github.com/gregorio-project/gregorio/issues/1766).
