...
- New score fonts with glyphs unique to Doninican chant.  These fonts replace the epiphonus and the augmented liquescents with corresponding figures from Dominican liturgical books.  Available fonts are `greciliae-op`, `gregorio-op`, and `parmesan-op`, based on their respective counterparts.  Select the desired font with the `\gresetgregoriofont` command.  See [#1](https://github.com/gregorio-project/gregorio/issues/1).
