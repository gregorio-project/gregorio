...
CENTER_NOT_DETERMINED means that the plugin has encountered no { nor } (in the gabc syntax).
CENTER_HALF_DETERMINED means that the plugin has encountered a { but no }, and we will try to determine a middle starting after the {.
CENTER_FULLY_DETERMINED means that lex has encountered a { and a }, so we won't determine the middle, it is considered done.
CENTER_DETERMINING_MIDDLE is used internally in the big function to know where we are in the middle determination.
